...
import com.example.handheld.atv.holder.adapters.listTrasMateriaPrimaSarAdapter;
...
import com.example.handheld.modelos.DetalleTranMateriaPrimaSarModelo;
...
public class EscanerTrasladoMateriaPrimaSar extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    List<DetalleTranMateriaPrimaSarModelo> ListaEscaner = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_escaner_traslado_materia_prima_sar);
    ...
    private void ingresarCedulas() {
        AlertDialog.Builder builder = new AlertDialog.Builder(EscanerTrasladoMateriaPrimaSar.this);
        ...
        btnAceptar.setOnClickListener(v12 -> {
            ...
            if (CeEntrega.equals("") || CeRecibe.equals("")){
                ...
                if (CeEntrega.equals(CeRecibe)){
                    ...
                    personaEntrega = conexion.obtenerPermisoPersonaTrasladoMateriaSar(EscanerTrasladoMateriaPrimaSar.this,CeEntrega,"entrega" );
                    ...
                    //personaRecibe = conexion.obtenerPermisoPersonaTrasladoMateriaSar(EscanerTrasladoMateriaPrimaSar.this,CeRecibe,"recibe" );
    ...
    public void consultarTipos(){
        ...
        tiposLista = conexion.obtenerTiposSar(getApplication());
        listaTp = obtenerLista(tiposLista);
        ArrayAdapter<CharSequence> adapter = new ArrayAdapter(EscanerTrasladoMateriaPrimaSar.this, android.R.layout.simple_spinner_item, listaTp);
    ...
    private void codigoIngresado(){
        ...
        if (validarCodigoBarrasTref(consecutivo)) {
            ...
            if (validarTrasladoTref(consecutivo)){
                ...
                String peso = conexion.valorTodo(EscanerTrasladoMateriaPrimaSar.this,sql_peso);
                String codigo = conexion.valorTodo(EscanerTrasladoMateriaPrimaSar.this,sql_codigo);
                ...
                if (validar_guardado_rollo(peso)){
                    if (codigo.equals(pcodigo)){
                        ...
                        lblDescripcion.setText(conexion.obtenerDescripcionCodigo(EscanerTrasladoMateriaPrimaSar.this,sql_descripcion));
                        txtKilosRollo.setText(peso);
                        etCodigo.setEnabled(false);
                        toastAcierto("Rollo validado");
                    }else{
                        toastError("El código del alambre no pertenece al pedido");
                        AudioError();
                        leer_nuevo();
                    }
                }else{
    ...
    private boolean validarTrasladoTref(String consecutivo) {
        ...
        String id_detalle = obj_gestion_alambronLn.extraerDatoCodigoBarrasTrefilacion("id_detalle", consecutivo);
        String id_rollo = obj_gestion_alambronLn.extraerDatoCodigoBarrasTrefilacion("id_rollo", consecutivo);
        ValidarTrasladoTrefModelo validacion = conexion.validarTrasladoTref(EscanerTrasladoMateriaPrimaSar.this,consecutivo_materia_prima,id_detalle,id_rollo);
    ...
    private boolean validarCodigoBarrasTref(String consecutivo) {
        ...
        if (!consecutivo_materia_prima.equals("") && !id_detalle.equals("") && !id_rollo.equals("")){
            ...
            String id = conexion.valorTodo(EscanerTrasladoMateriaPrimaSar.this,sql);
            if (!id.equals("")){
                resp = true;
            }
        }
        ...
        if (!lblCodigo.getText().toString().isEmpty() && !lblCodigo.getText().toString().equals("LEA CODIGO")){
            if(!txtKilosRollo.getText().toString().isEmpty() && !txtKilosRollo.getText().equals("")){
                if (!spinner.getSelectedItem().equals("Seleccione")){
                    if (conexion.existeCodigo(EscanerTrasladoMateriaPrimaSar.this,lblCodigo.getText().toString())){
                        if (Double.parseDouble(txtKilosRollo.getText().toString()) > 0){
                            if(conexion.existeTipoTransaccion(EscanerTrasladoMateriaPrimaSar.this,spinner.getSelectedItem().toString())){
    ...
    private void guardar() throws SQLException {
        ...
        double gStock = Double.parseDouble(conexion.consultarStock(EscanerTrasladoMateriaPrimaSar.this,gCodigo,gBodega));
        String sql_costo_unit = "select costo_unitario from referencias where codigo = '" + gCodigo + "'";
        Double gCosto_unit = Double.parseDouble(conexion.valorTodoCorsan(EscanerTrasladoMateriaPrimaSar.this,sql_costo_unit));
        ...
        if (gPeso <= gStock){
            ...
            //String sql_detalle_translado = "INSERT INTO " + db_produccion + "jd_detalle_traslado_materia_prima_sar (nit_entrega,nit_recibe,fecha_transaccion,trb1) " +
            //        "VALUES (" + personaEntrega.getNit() + "," + personaRecibe.getNit() + ",'" + fechaActualString + "'," + numero_transaccion + ") ";

            String sql_detalle_translado = "INSERT INTO " + db_produccion + "jd_detalle_traslado_materia_prima_sar (nit_entrega,fecha_transaccion,trb1) " +
            ...
            try {
                ...
                Toast.makeText(EscanerTrasladoMateriaPrimaSar.this, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
            ...
            String sql = "UPDATE " + db_produccion + "J_rollos_tref SET sar=" + numero_transaccion + " where  id_detalle=" + id_detalle + "  AND id_rollo=" + id_rollo + " AND cod_orden=" + consecutivo_materia_prima;
            try {
                //Se añade el sql a la lista
            ...
            error = ing_prod_ad.ExecuteSqlTransaction(listTransaccion_corsan, ConfiguracionBD.obtenerNombreBD(1), EscanerTrasladoMateriaPrimaSar.this);
            if(error.equals("")){
    ...
    private List<Object> traslado_bodega(String codigo, Double cantidad, String tipo, Double costo_unit){
        ...
        numero_transaccion = Integer.valueOf(Obj_ordenprodLn.mover_consecutivo(tipo, EscanerTrasladoMateriaPrimaSar.this));
        listSql = objTraslado_bodLn.listaTransaccionDatable_traslado_sar(numero_transaccion, codigo, bod_origen, calendar, notas, usuario, cantidad, tipo, costo_unit,EscanerTrasladoMateriaPrimaSar.this);
        return listSql;
    }
    ...
    public void addRollo(String Nro_orden,String Id_detalle,String Id_rollo, String gCodigo, String gPeso, String gTipo, String numero_transaccion){
        DetalleTranMateriaPrimaSarModelo detalleTranMateriaPrimaSarModelo;

        detalleTranMateriaPrimaSarModelo = new DetalleTranMateriaPrimaSarModelo();
        detalleTranMateriaPrimaSarModelo.setNro_orden(Nro_orden);
        detalleTranMateriaPrimaSarModelo.setId_detalle(Id_detalle);
        detalleTranMateriaPrimaSarModelo.setNumRollo(Id_rollo);
        detalleTranMateriaPrimaSarModelo.setCodigo(gCodigo);
        detalleTranMateriaPrimaSarModelo.setPeso(gPeso);
        detalleTranMateriaPrimaSarModelo.setTipoTransa(gTipo);
        detalleTranMateriaPrimaSarModelo.setNumTransa(numero_transaccion);
        ListaEscaner.add(detalleTranMateriaPrimaSarModelo);
    ...
    public void addrollotrans(){
        ...
        EscanerAdapter = new listTrasMateriaPrimaSarAdapter(EscanerTrasladoMateriaPrimaSar.this,R.layout.item_row_traslado_materiaprima_sar,ListaEscaner);
