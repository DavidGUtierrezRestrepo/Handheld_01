...
import com.example.handheld.atv.holder.adapters.listTrasMateriaPrimaSavAdapter;
...
import com.example.handheld.modelos.DetalleTranMateriaPrimaSavModelo;
...
public class EscanerTrasladoMateriaPrimaSav extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    List<DetalleTranMateriaPrimaSavModelo> ListaEscaner = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_escaner_traslado_materia_prima_sav);
    ...
    private void ingresarCedulas() {
        AlertDialog.Builder builder = new AlertDialog.Builder(EscanerTrasladoMateriaPrimaSav.this);
        ...
        btnAceptar.setOnClickListener(v12 -> {
            ...
            if (CeEntrega.equals("") || CeRecibe.equals("")){
                ...
                if (CeEntrega.equals(CeRecibe)){
                    ...
                    personaEntrega = conexion.obtenerPermisoPersonaTrasladoMateriaSav(EscanerTrasladoMateriaPrimaSav.this,CeEntrega,"entrega" );
                    ...
                    //personaRecibe = conexion.obtenerPermisoPersonaTrasladoMateriaSav(EscanerTrasladoMateriaPrimaSav.this,CeRecibe,"recibe" );
    ...
    public void consultarTipos(){
        ...
        tiposLista = conexion.obtenerTiposSav(getApplication());
        listaTp = obtenerLista(tiposLista);
        ArrayAdapter<CharSequence> adapter = new ArrayAdapter(EscanerTrasladoMateriaPrimaSav.this, android.R.layout.simple_spinner_item, listaTp);
    ...
    private void codigoIngresado(){
        ...
        if (validarCodigoBarrasTref(consecutivo)) {
            ...
            if (validarTrasladoTref(consecutivo)){
                ...
                String peso = conexion.valorTodo(EscanerTrasladoMateriaPrimaSav.this,sql_peso);
                String codigo = conexion.valorTodo(EscanerTrasladoMateriaPrimaSav.this,sql_codigo);
                ...
                if (validar_guardado_rollo(peso)){
                    if (codigo.equals(pcodigo)){
                        ...
                        lblDescripcion.setText(conexion.obtenerDescripcionCodigo(EscanerTrasladoMateriaPrimaSav.this,sql_descripcion));
    ...
    private boolean validar_guardado_rollo(String peso) {
        ...
        String sql_rollo = "SELECT (SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_Tsav_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN D.cantidad  ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_Tsav_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )As pendiente FROM J_salida_materia_prima_Tsav_enc E ,J_salida_materia_prima_Tsav_det D, CORSAN.dbo.referencias R where E.numero=" + pNumero + " AND E.anulado is null AND  R.codigo = D.codigo AND (e.devolver = 'N' OR e.devolver IS NULL ) AND E.numero = D.numero AND id_detalle = D.id_detalle";
        peso_solicitud = Double.parseDouble(conexion.valorTodo(EscanerTrasladoMateriaPrimaSav.this,sql_rollo));
    ...
    private boolean validarTrasladoTref(String consecutivo) {
        ...
        String id_detalle = obj_gestion_alambronLn.extraerDatoCodigoBarrasTrefilacion("id_detalle", consecutivo);
        String id_rollo = obj_gestion_alambronLn.extraerDatoCodigoBarrasTrefilacion("id_rollo", consecutivo);
        ValidarTrasladoTrefModelo validacion = conexion.validarTrasladoTref(EscanerTrasladoMateriaPrimaSav.this,consecutivo_materia_prima,id_detalle,id_rollo);
    ...
    private boolean validarCodigoBarrasTref(String consecutivo) {
        ...
        if (!consecutivo_materia_prima.equals("") && !id_detalle.equals("") && !id_rollo.equals("")){
            ...
            String id = conexion.valorTodo(EscanerTrasladoMateriaPrimaSav.this,sql);
            if (!id.equals("")){
                resp = true;
            }
        }
        ...
        if (!lblCodigo.getText().toString().isEmpty() && !lblCodigo.getText().toString().equals("LEA CODIGO")){
            if(!txtKilosRollo.getText().toString().isEmpty() && !txtKilosRollo.getText().equals("")){
                if (!spinner.getSelectedItem().equals("Seleccione")){
                    if (conexion.existeCodigo(EscanerTrasladoMateriaPrimaSav.this,lblCodigo.getText().toString())){
                        if (Double.parseDouble(txtKilosRollo.getText().toString()) > 0){
                            if(conexion.existeTipoTransaccion(EscanerTrasladoMateriaPrimaSav.this,spinner.getSelectedItem().toString())){
    ...
    private void guardar() throws SQLException {
        ...
        double gStock = Double.parseDouble(conexion.consultarStock(EscanerTrasladoMateriaPrimaSav.this,gCodigo,gBodega));
        String sql_costo_unit = "select costo_unitario from referencias where codigo = '" + gCodigo + "'";
        Double gCosto_unit = Double.parseDouble(conexion.valorTodoCorsan(EscanerTrasladoMateriaPrimaSav.this,sql_costo_unit));
        ...
        if (gPeso <= gStock){
            ...
            //String sql_detalle_translado = "INSERT INTO " + db_produccion + "jd_detalle_traslado_materia_prima_sav (nit_entrega,nit_recibe,fecha_transaccion,trb1) " +
            //        "VALUES (" + personaEntrega.getNit() + "," + personaRecibe.getNit() + ",'" + fechaActualString + "'," + numero_transaccion + ") ";

            String sql_detalle_translado = "INSERT INTO " + db_produccion + "jd_detalle_traslado_materia_prima_sav (nit_entrega,fecha_transaccion,trb1) " +
            ...
            try {
                ...
                Toast.makeText(EscanerTrasladoMateriaPrimaSav.this, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
            ...
            String sql = "UPDATE " + db_produccion + "J_rollos_tref SET sav=" + numero_transaccion + " where  id_detalle=" + id_detalle + "  AND id_rollo=" + id_rollo + " AND cod_orden=" + consecutivo_materia_prima;
            try {
                //Se añade el sql a la lista
            ...
            error = ing_prod_ad.ExecuteSqlTransaction(listTransaccion_corsan, ConfiguracionBD.obtenerNombreBD(1), EscanerTrasladoMateriaPrimaSav.this);
            if(error.equals("")){
    ...
    private List<Object> traslado_bodega(String codigo, Double cantidad, String tipo, Double costo_unit){
        ...
        numero_transaccion = Integer.valueOf(Obj_ordenprodLn.mover_consecutivo(tipo, EscanerTrasladoMateriaPrimaSav.this));
        listSql = objTraslado_bodLn.listaTransaccionDatable_traslado_sav(numero_transaccion, codigo, bod_origen, calendar, notas, usuario, cantidad, tipo, costo_unit,EscanerTrasladoMateriaPrimaSav.this);
        return listSql;
    }
    ...
    public void addRollo(String Nro_orden,String Id_detalle,String Id_rollo, String gCodigo, String gPeso, String gTipo, String numero_transaccion){
        DetalleTranMateriaPrimaSavModelo detalleTranMateriaPrimaSavModelo;

        detalleTranMateriaPrimaSavModelo = new DetalleTranMateriaPrimaSavModelo();
        detalleTranMateriaPrimaSavModelo.setNro_orden(Nro_orden);
        detalleTranMateriaPrimaSavModelo.setId_detalle(Id_detalle);
        detalleTranMateriaPrimaSavModelo.setNumRollo(Id_rollo);
        detalleTranMateriaPrimaSavModelo.setCodigo(gCodigo);
        detalleTranMateriaPrimaSavModelo.setPeso(gPeso);
        detalleTranMateriaPrimaSavModelo.setTipoTransa(gTipo);
        detalleTranMateriaPrimaSavModelo.setNumTransa(numero_transaccion);
        ListaEscaner.add(detalleTranMateriaPrimaSavModelo);
    ...
    public void addrollotrans(){
        ...
        EscanerAdapter = new listTrasMateriaPrimaSavAdapter(EscanerTrasladoMateriaPrimaSav.this,R.layout.item_row_traslado_materiaprima_sav,ListaEscaner);
