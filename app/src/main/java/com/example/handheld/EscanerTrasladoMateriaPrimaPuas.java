...
import com.example.handheld.atv.holder.adapters.listTrasMateriaPrimaPuasAdapter;
...
public class EscanerTrasladoMateriaPrimaPuas extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    List<DetalleTranMateriaPrimaPuasModelo> ListaEscaner = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_escaner_traslado_materia_prima_puas);
    ...
    private void ingresarCedulas() {
        AlertDialog.Builder builder = new AlertDialog.Builder(EscanerTrasladoMateriaPrimaPuas.this);
        ...
        txtCedulaRecibe.setHint("Operario Púas");
        ...
        btnAceptar.setOnClickListener(v12 -> {
            ...
            if (CeEntrega.equals("") || CeRecibe.equals("")){
                ...
                if (CeEntrega.equals(CeRecibe)){
                    ...
                    personaEntrega = conexion.obtenerPermisoPersonaTrasladoMateriaPrimaPuas(EscanerTrasladoMateriaPrimaPuas.this,CeEntrega,"entrega" );
    ...
    public void consultarTipos(){
        ...
        tiposLista = conexion.obtenerTiposPuas(getApplication());
        listaTp = obtenerLista(tiposLista);
        ArrayAdapter<CharSequence> adapter = new ArrayAdapter(EscanerTrasladoMateriaPrimaPuas.this, android.R.layout.simple_spinner_item, listaTp);
        ...
        if (validarCodigoBarrasGalv(consecutivo)) {
            ...
            if (validarTrasladoGalv(consecutivo)){
                ...
                String sql_peso = "select r.peso \n" +
                        "FROM D_rollo_galvanizado_f r , D_orden_pro_galv_enc e \n" +
                        "WHERE e.consecutivo_orden_G = r.nro_orden AND r.nro_orden=" + nro_orden + " AND r.consecutivo_rollo =" + id_rollo;
                String peso = conexion.valorTodo(EscanerTrasladoMateriaPrimaPuas.this,sql_peso);
                String codigo = conexion.valorTodo(EscanerTrasladoMateriaPrimaPuas.this,sql_codigo);
                ...
                if (validar_guardado_rollo(peso)){
                    if (codigo.equals(pcodigo)){
                        ...
                        lblDescripcion.setText(conexion.obtenerDescripcionCodigo(EscanerTrasladoMateriaPrimaPuas.this,sql_descripcion));
    ...
    private boolean validarTrasladoGalv(String consecutivo) {
        ...
        String nro_orden = obj_gestion_alambronLn.extraerDatoCodigoBarrasGalvanizado("nro_orden", consecutivo);
        String id_rollo = obj_gestion_alambronLn.extraerDatoCodigoBarrasGalvanizado("nro_rollo", consecutivo);
        ValidarTrasladoGalvModelo validacion = conexion.validarTrasladoGalv(EscanerTrasladoMateriaPrimaPuas.this,nro_orden,id_rollo);
    ...
    private boolean validarCodigoBarrasGalv(String consecutivo) {
        ...
        if (!nro_orden.equals("") && !id_rollo.equals("")){
            ...
            String id = conexion.valorTodo(EscanerTrasladoMateriaPrimaPuas.this,sql);
            if (!id.equals("")){
                resp = true;
            }
        }
        ...
        if (!lblCodigo.getText().toString().isEmpty() && !lblCodigo.getText().toString().equals("LEA CODIGO")){
            if(!txtKilosRollo.getText().toString().isEmpty() && !txtKilosRollo.getText().equals("")){
                if (!spinner.getSelectedItem().equals("Seleccione")){
                    if (conexion.existeCodigo(EscanerTrasladoMateriaPrimaPuas.this,lblCodigo.getText().toString())){
                        if (Double.parseDouble(txtKilosRollo.getText().toString()) > 0){
                            if(conexion.existeTipoTransaccion(EscanerTrasladoMateriaPrimaPuas.this,spinner.getSelectedItem().toString())){
    ...
    private void guardar() throws SQLException {
        ...
        double gStock = Double.parseDouble(conexion.consultarStock(EscanerTrasladoMateriaPrimaPuas.this,gCodigo,gBodega));
        String sql_costo_unit = "select costo_unitario from referencias where codigo = '" + gCodigo + "'";
        Double gCosto_unit = Double.parseDouble(conexion.valorTodoCorsan(EscanerTrasladoMateriaPrimaPuas.this,sql_costo_unit));
        ...
        if (gPeso <= gStock){
            ...
            //String sql_detalle_translado = "INSERT INTO " + db_produccion + "jd_detalle_traslado_materia_prima_puas (nit_entrega,nit_recibe,fecha_transaccion,trb1) " +
            //        "VALUES (" + personaEntrega.getNit() + "," + personaRecibe.getNit() + ",'" + fechaActualString + "'," + numero_transaccion + ") ";

            String sql_detalle_translado = "INSERT INTO " + db_produccion + "jd_detalle_traslado_materia_prima_puas (nit_entrega,fecha_transaccion,trb1) " +
            ...
            try {
                ...
                Toast.makeText(EscanerTrasladoMateriaPrimaPuas.this, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
            ...
            String sql = "UPDATE " + db_produccion + "D_rollo_galvanizado_f SET destino = 'A', traslado=" + numero_transaccion + " where  nro_orden =" + consecutivo_materia_prima + "  AND consecutivo_rollo =" + id_rollo;
            try {
                //Se añade el sql a la lista
            ...
            sql_solicitud = "INSERT INTO " + db_produccion + "J_salida_materia_prima_PU_transaccion \n" +
            ...
            error = ing_prod_ad.ExecuteSqlTransaction(listTransaccion_corsan, ConfiguracionBD.obtenerNombreBD(1), EscanerTrasladoMateriaPrimaPuas.this);
            if(error.equals("")){
                addRollo(consecutivo_materia_prima, id_rollo, gCodigo, Double.toString(gPeso), gTipo, numero_transaccion.toString());
                etCodigo.setEnabled(true);
                leer_nuevo();
                contar_movimientos();
                cargando.setVisibility(View.INVISIBLE);
    ...
    private List<Object> traslado_bodega(String codigo, Double cantidad, String tipo, Double costo_unit){
        ...
        numero_transaccion = Integer.valueOf(Obj_ordenprodLn.mover_consecutivo(tipo, EscanerTrasladoMateriaPrimaPuas.this));
        listSql = objTraslado_bodLn.listaTransaccionDatable_traslado_puas(numero_transaccion, codigo, bod_origen, calendar, notas, usuario, cantidad, tipo, costo_unit,EscanerTrasladoMateriaPrimaPuas.this);
        return listSql;
    }

    //Se añaden los datos del rollo en la lista "ListaEscaner"
    public void addRollo(String Nro_orden,String Id_rollo, String gCodigo, String gPeso, String gTipo, String numero_transaccion){
        DetalleTranMateriaPrimaPuasModelo detalleTranMateriaPrimaPuasModelo;

        detalleTranMateriaPrimaPuasModelo = new DetalleTranMateriaPrimaPuasModelo();
        detalleTranMateriaPrimaPuasModelo.setNro_orden(Nro_orden);
        detalleTranMateriaPrimaPuasModelo.setNumRollo(Id_rollo);
        detalleTranMateriaPrimaPuasModelo.setCodigo(gCodigo);
        detalleTranMateriaPrimaPuasModelo.setPeso(gPeso);
        detalleTranMateriaPrimaPuasModelo.setTipoTransa(gTipo);
        detalleTranMateriaPrimaPuasModelo.setNumTransa(numero_transaccion);
        ListaEscaner.add(detalleTranMateriaPrimaPuasModelo);
    ...
    public void addrollotrans(){
        ...
        EscanerAdapter = new listTrasMateriaPrimaPuasAdapter(EscanerTrasladoMateriaPrimaPuas.this,R.layout.item_row_traslado_materiaprima_puas,ListaEscaner);
